(ns tashikame.cacao
  "Agent-side CACAO issuance (JVM). The tashikame actor mints its OWN
  ...
  `tsumugu.cacao` (keep in sync); the SIWE/wire builders below are a faithful
  ...
  Per-actor key model: tashikame generates + persists its OWN Ed25519 key
...
(defn canonical-graph
  "The deterministic graph CID for one of tashikame's databases. The edge
...
(def default-db-name
  "tashikame's primary database — the verdict ledger."
  "factcheck")
...
(defn load-or-create-identity!
  "Per-actor key: load tashikame's persisted Ed25519 identity at `path`, or
